...
def replace_urls_in_text(text):
    ...
    return new_text
def replace_urls_in_text1(text):
    url_pattern = r'(https?://[^\s]+)'
    def replace_url(match):
        url = match.group(1)
        try:
            ...
            response = requests.post(urlPost, json=payload)
            response_data = response.json()
            processed_text = response_data["processedText"]
        ...
        while True:
            try:
                ...
                for message in reversed(messages):
                    ...
                    if modified_text:
                        modified_text = replace_urls_in_text1(modified_text)
                        if keywords:
